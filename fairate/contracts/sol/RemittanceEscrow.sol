...
contract RemittanceEscrow is ASCBase {
    ...
    function _processRelease(bytes32 queryId, bytes memory encodedTransaction) internal {
        ...
        // Day 6 will scale `amount` by the attested Chainlink rate here instead of paying 1:1.
        FairateMintableToken(payoutToken).mint(receiver, amount);

        _recordReputation(sender, receiver, amount);

        emit TransferReleased(sender, receiver, amount, depositId, queryId);
    }
